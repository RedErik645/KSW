...
public class HyperSpline {
    ...
    /* JADX INFO: Multiple debug info for r4v21 android.support.constraint.motion.utils.HyperSpline$Cubic[]: [D('i' int), D('C' android.support.constraint.motion.utils.HyperSpline$Cubic[])] */
    static Cubic[] calcNaturalCubic(int n, double[] x) {
        ...
        for (int i2 = 1; i2 < n2; i2++) {
            delta[i2] = (((x[i2 + 1] - x[i2 - 1]) * 3.0d) - delta[i2 - 1]) * gamma[i2];
        }
        ...
        for (int i4 = 0; i4 < n2; i4++) {
            C[i4] = new Cubic((double) ((float) x[i4]), D[i4], (((x[i4 + 1] - x[i4]) * 3.0d) - (D[i4] * 2.0d)) - D[i4 + 1], ((x[i4] - x[i4 + 1]) * 2.0d) + D[i4] + D[i4 + 1]);
        }
        return C;
    }
