...
public final class BarUtils {
    ...
    public static void setStatusBarColor(View fakeStatusBar, int color) {
        ...
        } else if (Build.VERSION.SDK_INT >= 19 && (activity = UtilsBridge.getActivityByContext(fakeStatusBar.getContext())) != null) {
            transparentStatusBar(activity);
        ...
        if (window != null) {
            ...
            while (true) {
                if (i < count) {
                    View child = decorView.getChildAt(i);
                    int id = child.getId();
                    if (id != -1 && "navigationBarBackground".equals(getResNameById(id)) && child.getVisibility() == 0) {
                        isVisible = true;
                        break;
                    }
                    i++;
