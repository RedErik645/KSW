...
public final class RSSExpandedReader extends AbstractRSSReader {
    ...
    /* JADX INFO: Multiple debug info for r12v6 int: [D('checksumPortion' int), D('i' int)] */
    /* JADX INFO: Multiple debug info for r1v1 int: [D('counters' int[]), D('evenWidest' int)] */
    /* JADX INFO: Multiple debug info for r0v6 int: [D('value' int), D('vEven' int)] */
    /* access modifiers changed from: package-private */
    public DataCharacter decodeDataCharacter(BitArray row, FinderPattern pattern, boolean isOddPattern, boolean leftChar) throws NotFoundException {
        ...
        if (Math.abs(elementWidth - expectedElementWidth) / expectedElementWidth <= 0.3f) {
            ...
            return new DataCharacter((RSSUtils.getRSSvalue(oddCounts, oddWidest, true) * EVEN_TOTAL_SUBSET[group]) + RSSUtils.getRSSvalue(evenCounts, 9 - oddWidest, false) + GSUM[group], i5);
        }
        throw NotFoundException.getNotFoundInstance();
    }
