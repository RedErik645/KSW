...
public class WitsStatus {
    ...
    public static void sendOutBtStatus(BtPhoneStatus btPhoneStatus) {
        PowerManagerApp.sendStatus(new WitsStatus(3, new Gson().toJson(btPhoneStatus)));
    }

    public static void sendOutMusicStatus(MusicStatus musicStatus) {
        PowerManagerApp.sendStatus(new WitsStatus(21, new Gson().toJson(musicStatus)));
    }

    public static void sendOutVideoStatus(VideoStatus videoStatus) {
        PowerManagerApp.sendStatus(new WitsStatus(22, new Gson().toJson(videoStatus)));
    }

    public static void sendOutPictureStatus(PictureStatus pictureStatus) {
        PowerManagerApp.sendStatus(new WitsStatus(23, new Gson().toJson(pictureStatus)));
    }
