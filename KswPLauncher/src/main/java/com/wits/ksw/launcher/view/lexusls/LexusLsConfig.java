...
public class LexusLsConfig {
    ...
    public static void factoryAppControlStatus() {
        Iterator<LexusLsAppSelBean> it;
        Log.i("ContentValues", "factoryAppControlStatus");
        if (UiThemeUtils.isLEXUS_LS_UI(KswApplication.appContext)) {
            it = mAppList.iterator();
        } else {
            it = mAppList_V2.iterator();
        }
        while (it.hasNext()) {
            try {
                LexusLsAppSelBean selBean = it.next();
                if (selBean != null) {
                    String cardName = selBean.getAppPkg();
                    if ("DTV_Type".equals(cardName)) {
                        if (PowerManagerApp.getSettingsInt("DTV_Type") != 1) {
                            it.remove();
                        }
                    } else if ("DVR_Type".equals(cardName)) {
                        if (PowerManagerApp.getSettingsInt("DVR_Type") != 1) {
                            it.remove();
                        }
                    } else if ("AUX_Type".equals(cardName)) {
                        if (PowerManagerApp.getSettingsInt("AUX_Type") != 1) {
                            it.remove();
                        }
                    } else if ("Front_view_camera".equals(cardName) && PowerManagerApp.getSettingsInt("Front_view_camera") != 1) {
                        it.remove();
                    }
                }
