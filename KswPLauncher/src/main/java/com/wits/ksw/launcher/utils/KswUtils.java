...
public class KswUtils {
    ...
    public static int calculateTranslate(int top, int h, int i, Context context) {
        ...
        int result = (h2 - Math.abs(h2 - top)) / 6;
        if (i == 1) {
            result = (top * 20) / 107;
        } else if (i == 2) {
    ...
    public static String formatMonth(Date date) {
        ...
        if (UiThemeUtils.isUI_PEMP_ID8(KswApplication.appContext)) {
            months = KswApplication.appContext.getResources().getStringArray(R.array.all_months);
        } else {
            months = KswApplication.appContext.getResources().getStringArray(R.array.ksw_id7_months);
        }
        return months[Integer.valueOf(m).intValue() - 1];
    }
    ...
    public static void sendKeyDownUpSync(final int key) {
        new Handler().post(new Runnable() {
            /* class com.wits.ksw.launcher.utils.KswUtils.AnonymousClass1 */

            public void run() {
                new Thread(new Runnable() {
                    /* class com.wits.ksw.launcher.utils.KswUtils.AnonymousClass1.AnonymousClass1 */
