...
public class ID8LauncherConstants {
    ...
    public static void IteratorNameList(List<String> nameList2) {
        try {
            ...
            while (it.hasNext()) {
                ...
                if (cardName.contains("DTV_Type") && PowerManagerApp.getSettingsInt("DTV_Type") != 1) {
                    it.remove();
                } else if (cardName.contains("DVR_Type") && PowerManagerApp.getSettingsInt("DVR_Type") != 1) {
                    it.remove();
                } else if (cardName.contains("AUX_Type") && PowerManagerApp.getSettingsInt("AUX_Type") != 1) {
                    it.remove();
                } else if (cardName.contains("Front_view_camera") && PowerManagerApp.getSettingsInt("Front_view_camera") != 1) {
                    it.remove();
                }
    ...
    public static boolean changeLeftIcon(int viewId, String name) {
        ...
        switch (viewId) {
            case R.id.ll_left_2 /*{ENCODED_INT: 2131231955}*/:
                leftIcon2 = name;
