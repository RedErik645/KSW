...
public class McuImpl {
    ...
    private McuImpl() {
        ...
        KswApplication.appContext.getContentResolver().registerContentObserver(Settings.System.getUriFor(KeyConfig.TempUnit), false, new ContentObserver(new Handler()) {
            /* class com.wits.ksw.launcher.model.McuImpl.AnonymousClass1 */
        ...
        KswApplication.appContext.getContentResolver().registerContentObserver(Settings.System.getUriFor(KeyConfig.FUEL_UNIT), false, new ContentObserver(new Handler()) {
            /* class com.wits.ksw.launcher.model.McuImpl.AnonymousClass2 */
    ...
    public void setDoor(McuStatus.CarData carDatam) {
        ...
        } else if (isM700BenzUi()) {
            setM700BenzDoorState(carDatam);
        } else {
    ...
    private boolean isM700BenzUi() {
        String display = Build.DISPLAY;
        if (TextUtils.isEmpty(display) || !display.contains("M700")) {
            return false;
        }
        if (!UiThemeUtils.isBenz_GS(KswApplication.appContext) && !UiThemeUtils.isBenz_NTG5(KswApplication.appContext) && !UiThemeUtils.isBenz_NTG6(KswApplication.appContext) && !UiThemeUtils.isBenz_MBUX(KswApplication.appContext) && !UiThemeUtils.isBenz_MBUX_2021(KswApplication.appContext) && !UiThemeUtils.isUI_KSW_MBUX_1024(KswApplication.appContext) && !UiThemeUtils.isBenz_MBUX_2021_KSW(KswApplication.appContext) && !UiThemeUtils.isBenz_MBUX_2021_KSW_V2(KswApplication.appContext) && !UiThemeUtils.isBenz_NTG6_FY(KswApplication.appContext) && !UiThemeUtils.isUI_NTG6_FY_V2(KswApplication.appContext) && !UiThemeUtils.isUI_NTG6_FY_V3(KswApplication.appContext)) {
            return false;
        }
        return true;
    }

    private void setM700BenzDoorState(McuStatus.CarData carDatam) {
        if (carDatam == null) {
            ExceptionPrint.print("setDoor: McuStatus.CarData is null");
            return;
        }
        ...
        this.carInfoMutableLiveData.postValue(this.carInfo);
        Log.i("KswApplication", "setDoor: doorCount=" + doorCount + " : 驾驶门:" + LEFT_AHEAD + " 副驾驶门:" + RIGHT_AHEAD + " 驾驶员后门:" + LEFT_BACK + " 副驾驶员后门:" + RIGHT_BACK + " 后备箱门:" + BACK_COVER);
    }
