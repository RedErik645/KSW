...
public class BwmNbtModel extends LauncherViewModel implements View.OnFocusChangeListener {
    ...
    private void focusChange(View v, boolean hasFocus) {
        switch (v.getId()) {
            case R.id.app_ll /*{ENCODED_INT: 2131230811}*/:
                if (hasFocus) {
                    refrshBg(0);
                    return;
                }
                return;
            case R.id.dashbroad_ll /*{ENCODED_INT: 2131231389}*/:
                if (hasFocus) {
                    refrshBg(6);
                    return;
                }
                return;
            case R.id.music_ll /*{ENCODED_INT: 2131232155}*/:
                if (hasFocus) {
                    refrshBg(2);
                    return;
                }
                return;
            case R.id.navi_ll /*{ENCODED_INT: 2131232178}*/:
                if (hasFocus) {
                    refrshBg(1);
                    return;
                }
                return;
            case R.id.phone_ll /*{ENCODED_INT: 2131232258}*/:
                if (hasFocus) {
                    refrshBg(4);
                    return;
                }
            ...
            case R.id.rl_settings /*{ENCODED_INT: 2131232480}*/:
                ...
                return;
            case R.id.video_ll /*{ENCODED_INT: 2131232852}*/:
                if (hasFocus) {
                    refrshBg(3);
                    return;
                }
