...
public class PluralFormat extends UFormat {
    ...
    /* JADX INFO: Multiple debug info for r5v3 int: [D('partIndex' int), D('type' com.ibm.icu.text.MessagePattern$Part$Type)] */
    ...
    private String format(Number numberObject, double number) {
        ...
        if (numberFormat2 instanceof DecimalFormat) {
            LocalizedNumberFormatter f2 = ((DecimalFormat) numberFormat2).toNumberFormatter();
            if (this.offset == 0.0d) {
                result = f2.format(numberObject);
            } else {
                result = f2.format(numberMinusOffset);
            }
            numberString = result.toString();
            f = result.getFixedDecimal();
        } else {
            if (d == 0.0d) {
            ...
            f = new PluralRules.FixedDecimal(numberMinusOffset);
        }
        int partIndex = findSubMessage(this.msgPattern, 0, this.pluralRulesWrapper, f, number);
