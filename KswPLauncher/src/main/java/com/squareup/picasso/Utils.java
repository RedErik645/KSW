...
public final class Utils {
    ...
    static long calculateDiskCacheSize(File dir) {
        long size = Constants.NTG6V3_PIC_FILE_MAX_LENGTH;
        try {
            StatFs statFs = new StatFs(dir.getAbsolutePath());
            size = (((long) statFs.getBlockCount()) * ((long) statFs.getBlockSize())) / 50;
        } catch (IllegalArgumentException e) {
        }
        return Math.max(Math.min(size, 52428800L), (long) Constants.NTG6V3_PIC_FILE_MAX_LENGTH);
    }
    ...
    /* JADX INFO: Multiple debug info for r2v11 int: [D('id' int), D('type' java.lang.String)] */
