...
final class RunSuspend implements Continuation<Unit> {
    ...
    public final Result<Unit> m1196getResultxLWZpok() {
        return this.result;
    }
    ...
    public void resumeWith(Object result2) {
        synchronized (this) {
            this.result = Result.m8boximpl(result2);
            notifyAll();
    ...
    public final void await() {
        synchronized (this) {
            while (true) {
                Result result2 = m1196getResultxLWZpok();
                if (result2 == null) {
                    wait();
                } else {
                    ResultKt.throwOnFailure(result2.m18unboximpl());
                }
            }
        }
    }
