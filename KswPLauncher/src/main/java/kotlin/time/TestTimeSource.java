...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1421plusAssignLRDsOJo(long duration) {
        long newReading;
        long longDelta = Duration.m1339toLongimpl(duration, getUnit());
        if (longDelta == Long.MIN_VALUE || longDelta == LongCompanionObject.MAX_VALUE) {
            double newReading2 = ((double) this.reading) + Duration.m1336toDoubleimpl(duration, getUnit());
            if (newReading2 > 9.223372036854776E18d || newReading2 < -9.223372036854776E18d) {
                m1420overflowLRDsOJo(duration);
            }
    ...
    private final void m1420overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + "ns is advanced by " + ((Object) Duration.m1342toStringimpl(duration)) + '.');
    }
}
