...
class FilesKt__UtilsKt extends FilesKt__FileTreeWalkKt {
    ...
    public static final boolean copyRecursively(File $this$copyRecursively, File target, boolean overwrite, Function2<? super File, ? super IOException, ? extends OnErrorAction> function2) {
        ...
        try {
            Iterator<File> it = FilesKt.walkTopDown($this$copyRecursively).onFail(new FilesKt__UtilsKt$copyRecursively$2(function2)).iterator();
            while (it.hasNext()) {
                File src = it.next();
                if (src.exists()) {
    ...
    private static final List<File> normalize$FilesKt__UtilsKt(List<? extends File> list) {
        ...
        for (File file : list) {
            ...
            if (!Intrinsics.areEqual(name, ".")) {
                if (!Intrinsics.areEqual(name, "..")) {
                    list2.add(file);
                } else if (list2.isEmpty() || Intrinsics.areEqual(((File) CollectionsKt.last(list2)).getName(), "..")) {
                    list2.add(file);
                } else {
